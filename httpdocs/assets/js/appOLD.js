...
var trendy = Foundation.utils.S("#trending-now");
...
$(document).ready(function() {
	...
	var ad = {
		home: {
			...
			atf_original: '<script type="text/javascript" src="http://ad.doubleclick.net/N4403/adj/simpledish.com/home;sect=home;sz=300x1050;dc_ref='+encodeURIComponent(location.href)+';ord='+Math.floor(Math.random() * 1e16)+'?"><\/script>',
		...
		article: {
			...
			atf: '<script language="javascript1.1" src="http://adserver.adtechus.com/addyn/3.0/5470.1/3214370/0/170/ADTECH;loc=100;target=_blank;key=key1+key2+key3+key4;grp=[group];misc='+new Date().getTime()+'"><\/script>',
	...
	iFrameResize({
			...
			resizedCallback: function(messageData){
				...
				if(label === 'atf' && messageData.height > 500) {
					...
					else if(label === 'header' && messageData.height < 80) {Foundation.utils.S("#header-ad").css("min-height", messageData.height);}					if(messageData.width < 500) {classToAdd += ' ad320';}
