...
/*END Slider Functions */
	...
	$('#featured-article-cont, #recipe-box-cont, #search-header-cont, #ad-300-atf-cont, #ad-300-btf2-cont').appendAround();
			...
			switch(thisId){
				...
				case "twitter":
					...
					twitterUrl = twitterUrl.replace('{twitterurl}', thisUrl).replace('{twittervia}', twitterHandle).replace('{twittertext}', twitterText.split(' ').join('+'));
					 window.open(twitterUrl, "popupWindow", "width=850,height=600,scrollbars=yes");
					break;
				...
				case "pinterest":
					...
					window.open(pinterestUrl, "popupWindow", "width=850,height=600,scrollbars=yes");
					break;

				case "google-plus":
					var googlePlusUrl = "https://plus.google.com/share?url=" + thisUrl;
					//window.open(googlePlusUrl, '_blank');
					window.open(googlePlusUrl, "popupWindow", "width=850,height=600,scrollbars=yes");
					break;
	...
	$('#social-buttons .social-button-v2').each(function(){
		var elem = $(this),
		thisId = $(elem).attr('id'),
		thisUrl = $(elem).attr('data-url'),
		thisTitle = $(elem).attr('data-title'),
...
// 300 x 250 ads
		if(document.getElementById('ad-300-atf-cont')){
		...
		if(document.getElementById('ad-300-btf1-cont')){
		...
		if(document.getElementById('ad-300-btf2-cont')){
		...
		if(document.getElementById('ad-300-btf3-cont')){
			document.getElementById('ad-300-btf3-cont').appendChild(document.getElementById('ad4_footer'));
...
// 728 x 90 ads
		if($('#ad-728-atf').length > 0){
			if($('#ad5_footer').length > 0){
				document.getElementById('ad-728-atf').appendChild(document.getElementById('ad5_footer'));
		...
		if($('#ad-728-btf2').length > 0){
			...
			$('#ad-728-btf2').append($('#ad6_footer'));
			$('#ad6_footer').css('display', 'inline');
...
<?php } ?>
	...
	//if (!seenMySDPopup && cookiesEnabled && !($('body').hasClass('bp0')) ) {
	//    //	Show Modal Window (SDPopUp)
	//	if($('#my-sd-promo')){
	//		console.log($('body').hasClass('bp0'));
	//		$('#my-sd-promo').SDPopUp({
	//			'useCookie' : true,
