...
class MPArticle{
	...
	public function getSearchResultsByNamesArray($args = []){
		$options = array_merge([
			...
			INNER JOIN ( 
				SELECT articles.*, i.article_post_img, i.article_preview_img, nc.cat_id, nc.cat_name, nc.cat_dir_name, nc.lft, nc.rgt, article_contributors.contributor_id, article_contributors.contributor_name, article_contributors.contributor_image, article_pages.article_page_assets_directory, article_pages.article_page_full_url  
				FROM articles  

				INNER JOIN (article_pages, article_page_categories, article_images as i, article_categories, categories as nc, articles_featured as featured, article_contributor_articles, article_contributors) 
				ON articles.article_id = i.article_id 
				AND articles.article_id = article_categories.article_id  
				AND article_pages.article_page_id = article_page_categories.article_page_id  
	...
	public function getTodaysFavorites(){
		$queryString = "SELECT article.*, parent.cat_id as parent_id, parent.cat_name as parent_name, parent.cat_dir_name as parent_dir_name 
			...
			INNER JOIN ( SELECT articles.article_id as a_id, articles.article_title as article_title, articles.article_seo_title, articles.article_status, categories.*, article_images.* 
							...
							WHERE article_categories.cat_id IN (8, 9, 11, 900)
...
//		echo htmlspecialchars($qString);
	...
	public function getByName($args = []){
		$options = array_merge([
			...
			$s = 'SELECT articles.*, article_contributors.contributor_id, article_contributors.contributor_name, article_contributors.contributor_image, 
	...
	public function getArticleRSS($args = []){
		$options = array_merge([
			...
			INNER JOIN ( 
				SELECT articles.*, i.article_post_img, i.article_preview_img, nc.cat_id, nc.cat_name, nc.cat_dir_name, nc.lft, nc.rgt, article_contributors.contributor_id, article_contributors.contributor_name, article_contributors.contributor_image  
				FROM articles  

				INNER JOIN (article_images as i, article_categories, categories as nc, articles_featured as featured, article_contributor_articles, article_contributors) 
				ON articles.article_id = i.article_id 
				AND articles.article_id = article_categories.article_id  
				AND article_categories.cat_id = nc.cat_id 
				AND articles.article_id = article_contributor_articles.article_id 
...
//	public function count_all_filtered($post_date, $assoc_cat, $visible='') {
	public function count_all_filtered() {
		
		$s = "SELECT count( DISTINCT article_id) as simpledish_article_count
	...
	public  function get_filtered($limit = 10, $assoc_cat = "", $post_date, $visible = '', $offset) {
		$category_arry = explode(' ',trim($assoc_cat));
		$first_category = $category_arry[0]; // will print Test			
		$SQL_like_category = '%'.$first_category.'%';			
		if ($post_date=='all' OR !isset($post_date)){
			$date_sql = "";			
		} else if (isset($post_date)) {
			$date_array = explode('_', $post_date);
			$month = intval($date_array[0]);
			$year = $date_array[1];
			$date_sql = "AND MONTH(post_date) = ".$month." AND YEAR(post_date) = ".$year;
		}
		if ($visible == '' OR !isset($visible)) {
			$status_sql = "";
		} else {
			$status_sql = "AND visible=".$visible;
		}
		...
		$q = $this->performQuery(['queryString' => $s]);
		return $q;
		// return self::find_by_sql("SELECT * FROM articles WHERE assoc_cat LIKE '{$SQL_like_category}' {$date_sql} {$status_sql} 
