...
class MPNavigation{
	...
	private function displayDropDownArticle($article){
		...
		$articleString .= '<div id="image-recipe">';
			$articleString .='<a href="'.$link.'" >';
				$articleString .= '<img src="'.$this->config['image_url'].'/articlesites/simpledish/preview/'.$article['article_preview_img'].'" alt="'.$article['article_title'].' Preview Image" />';
			$articleString .='</a>';
		$articleString .= '</div>';
		...
		$result.='<li class="parent" id="c901">';
	        ...
	        $result.='<div style="display: none;" class="submenu-wrapper">';
				...
				$result.='<div class="right">';
					$result.='<h2>20% OFF All COFFEE MUGS!</h2>';
					$result.='<div data-desc="20% OFF all coffee mugs! Use Code: LOVE  Ends 2/14" data-title="20% OFF all coffee mugs! Use Code: LOVE  Ends 2/14" class="article-info">';
						$result.='<div id="image-recipe">';
							$result.='<a target="_blank" href="http://shop.simpledish.com"><img alt="Shop Simpledish Preview Image" src="http://images.simpledish.com/articlesites/simpledish/campaign/Drop-Down-Ad.jpg"></a>';
						$result.='</div>';
	...
	public function getAllCategoriesWithArticles(){
		...
		$q = $pdo->query("SELECT 
		   t1.cat_id, t1.cat_name, t1.cat_dir_name
		FROM 
		    categories AS t1
		WHERE lft>1
		AND has_children = 0 
